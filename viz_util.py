...
def xyzwhl2eight(xyzwhl):
    ...
    box8 = np.array([
        [x+w/2, x+w/2, x-w/2, x-w/2, x+w/2, x+w/2, x-w/2, x-w/2 ],
        [y-h/2, y+h/2, y+h/2, y-h/2, y-h/2, y+h/2, y+h/2, y-h/2 ],
        [z-l/2, z-l/2, z-l/2, z-l/2, z+l/2, z+l/2, z+l/2, z+l/2 ]
        ])
    return box8.T

def draw_xyzwhl(gt_boxes3d, fig, color=(1,1,1), line_width=1, draw_text=True, text_scale=(1,1,1), color_list=None, rot=False):
    ''' Draw 3D bounding boxes
    Args:
        gt_boxes3d: numpy array (n,8,3) for XYZs of the box corners
        fig: mayavi figure handler
